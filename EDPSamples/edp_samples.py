...
def get_IRI2020_EDP(DateTime: str,
   ...
   # ---------------------------------------------------------
    ...
    assert os.path.exists(apf107_src), f"The file apf107.dat was not found at {apf107_src}"
    assert os.path.exists(ig_rz_src), f"The file ig_rz.dat was not found at {ig_rz_src}"
    ...
    cmd += f"{exe} {namelist_filename} {IRI_output_filename} > iri2020_namelist_driver.log"
    #print(cmd)
    ...
    # 4. Clean up symlinks 
    os.system(f"cd {Run_Folder};rm -f mcsat*.dat dgrf*.dat igrf*.dat *.asc apf107.dat ig_rz.dat")
...
def write_IRI2020_namelist(DateTime: str,
    ...
    Run_Folder = os.getenv("Tomography_Run_Folder")
    with open(Run_Folder+"/"+namelist_filename, 'w') as f:
        f.write('&EDPSamples\n')
...
def read_IRI2020_binary_output(IRI_output_filename: str) -> tuple(np.ndarray,np.ndarray):
    Run_Folder = os.getenv("Tomography_Run_Folder")
    with open(Run_Folder+"/"+IRI_output_filename, 'rb') as f:
        data = f.read()
    ...
    expected_size = 12 + (npts* nSample * (nheight+13) * 4)
...
class EDPSamples(xr.Dataset):
    ...
    **Coordinates / variables**
    ...
    def show_edps_feature(self):
        ...
        iri_parameters = self.sampling_parameters
        feature_edps = self.feature_edps
        DateTime = self.attrs['DateTime']
        ...
        fig.suptitle(f"Distribution of Input Parameters and Key EDP Characteristics for {DateTime}",
