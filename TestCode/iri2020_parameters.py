...
def read_apf107(file_path):
    ...
    apf107 = {
        "yr": np.array([], dtype=int),
        "mn": np.array([], dtype=int),
        "dy": np.array([], dtype=int),
        "iapda": np.array([], dtype=int),
        "iiap": np.empty((0, 8), dtype=int),
        "ir": np.array([], dtype=int),
        "f107": np.array([], dtype=float),
        "f107_81": np.array([], dtype=float),
        "f107_365": np.array([], dtype=float)
    }
    with open(filename, 'r') as f:
        for line in f:
            ...
            apf107["yr"] = np.append(apf107["yr"], yy)
            apf107["mn"] = np.append(apf107["mn"], int(line[3:6]))
            apf107["dy"] = np.append(apf107["dy"], int(line[6:9]))
            apf107["iapda"] = np.append(apf107["iapda"], int(line[9:12]))
            # iiap is a (n,8) array: append a new row
            iiap_row = np.array([[int(line[12 + i * 3:15 + i * 3]) for i in range(8)]], dtype=int)
            apf107["iiap"] = np.vstack([apf107["iiap"], iiap_row])
            apf107["ir"] = np.append(apf107["ir"], int(line[36:39]))
            apf107["f107"] = np.append(apf107["f107"], float(line[39:44]))
            apf107["f107_81"] = np.append(apf107["f107_81"], float(line[44:49]))
            apf107["f107_365"] = np.append(apf107["f107_365"], float(line[49:54]))
...
def read_ig_rz(file_path):
    ...
    revision_date = np.array([int(x) for x in lines[0].split(",") if x.strip().isdigit()])
    start_end_date = np.array([int(x) for x in lines[1].split(",") if x.strip().isdigit()])
...
def show_iri_inputs(apf107,ig_rz):
    ...
    pn = axs[0]
    # Loop over each "column" of apf107["iiap"] and plot each column
    # apf107["iiap"] is a list of lists, so we need to plot each index in range(len(apf107["iiap"][0]))
    iiap_arr = np.array(apf107["iiap"])  # shape: (N, 8)
    for i in range(iiap_arr.shape[1]):
        pn.plot(datenum, iiap_arr[:, i], marker='.', label=f'iiap[{i}]', linestyle='None')
    pn.plot(datenum, apf107["iapda"], marker='o', label=f'iapda]', linestyle='None',color=(0,0,1))
    pn.legend(loc='upper right', fontsize='x-small', ncol=2)
