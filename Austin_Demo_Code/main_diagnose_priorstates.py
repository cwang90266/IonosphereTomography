...
from test_param_iono import (
    EKF_Param, select_arcs_by_count_bin, _get_reflection_height, _fit_power_law, plot_initial_ekf_error_covariance,
)
...
from Ionosphere_Tomography_Inverter.Ionophy_Tomography_Inverter import fill_kf_topside_using_ekf_shape
...
from ekf_param_correlation_config import (
    EKF_PARAM_CORR_TUNING,
    tune_all_parameter_covariances,
)
# from isr_aid_covariance import _fit_esr_8params, _tune_param_covariance_with_esr
import warnings
...
# NOTE: Janathon claimed that the sigma should be ~1 TECu. but Chunming stated that the 10 TECu setup is becuz of the interpolation. 
# Each Ne is barycentric interpolated by 6 points. And Ne is finite discrete smaples of 500 segments instead of continuous like RO TEC does.
...
EKF_PRIOR_SCALE  = 1.0     # multiplier on prior param variances P_b, a factor for initial err cov
...
# ── ISR covariance calibration experiment ─────────────────────────────────────
EKF_ESR_COV_TUNING = True

# Strength of the ISR-derived covariance correction.
# 0.0 = original covariance
# 1.0 = add the full ESR-vs-IRI error outer product
EKF_ESR_COV_WEIGHT = 0.2

# ============================================================
# EKF covariance source
#
# True:
#   use manually defined pairwise correlations from
#   ekf_param_correlation_config.py
#
# False:
#   use the default IRI-generated parameter covariance
# ============================================================

EKF_USE_MANUAL_PARAM_CORR = False
...
ISR_ROI_MAX_KM         = 2000.0   # RO peak-tangent-point → ISR site gate (great-circle)
...
OCC_COUNT_BINS = [None] #[None, 55, 45, 35, 25, 15, 5]
...
def select_priority_days(force: bool = False) -> list[dict]:
    ...
    # days = isr_days(edps)
    days = [
    d for d in isr_days(edps)
    if d == _dt.date(2025, 11, 22)]
...
def _build_hour_edp_cache(t_centre: pd.Timestamp) -> dict:
    ...
    args = (
        ...
        1000, 5.0, 5.0, str(_GLOBAL_EDP_DATA_DIR),
...
def plot_ekf_ro_tec_truth_prior_post(
    arc_truth_list,
    ekf_result,
    kf_result,
    save_dir,
    ...
    """
    One figure per RO event:

    Left:
        black dots = EKF horizontal voxels
        red line   = RO tangent-point track
    ...
    ekf_slices = ekf_result["tec_slices"]

    # Prefer joint KF posterior if available
    kf_slices = kf_result.get(
        "joint_tec_slices",
        kf_result.get("tec_slices", [])
    )

    grid_lats = np.asarray(ekf_result["grid_lats"], dtype=float)
    grid_lons = np.asarray(ekf_result["grid_lons"], dtype=float)
    ...
    for i, (arc, sl) in enumerate(zip(arc_truth_list, ekf_slices)):

        # Skip IGS arcs
        if str(arc.get("conid", "")).upper() == "IGS":
            continue
        ...
        tang = np.asarray(sl["tang_km"], dtype=float)
        ...
        # Keep all arrays at same length
        n = min(
            len(tang),
            len(truth),
            len(ekf_prior),
        ...
        valid = (
            np.isfinite(tang)
            & np.isfinite(truth)
            & np.isfinite(ekf_prior)
            & np.isfinite(ekf_post)
            & np.isfinite(kf_prior)
            & np.isfinite(kf_post)
        )
        ...
        tang      = tang[valid]
        truth     = truth[valid]
        ekf_prior = ekf_prior[valid]
        ekf_post  = ekf_post[valid]
        kf_prior  = kf_prior[valid]
        kf_post   = kf_post[valid]

        # Sort all TEC curves by tangent altitude
        order = np.argsort(tang)

        tang      = tang[order]
        truth     = truth[order]
        ekf_prior = ekf_prior[order]
        ekf_post  = ekf_post[order]
        kf_prior  = kf_prior[order]
        kf_post   = kf_post[order]
        ...
        gs = GridSpec(
            1, 3,
            figure=fig,
            width_ratios=[1.35, 2.0, 1.0],
            wspace=0.18,
        )

        # ============================================================
        # LEFT: voxel + RO tangent-point map
        # ============================================================
        ...
        # Centre projection approximately on this domain
        all_lat = np.concatenate([
            grid_lats[np.isfinite(grid_lats)],
            tp_lats[np.isfinite(tp_lats)],
        ])

        all_lon = np.concatenate([
            grid_lons[np.isfinite(grid_lons)],
            tp_lons[np.isfinite(tp_lons)],
        ])

        if all_lat.size:
            centre_lat = float(np.nanmean(all_lat))
        else:
            centre_lat = 45.0

        if all_lon.size:
            centre_lon = float(np.nanmean(all_lon))
        else:
            centre_lon = 0.0
        ...
        # RO tangent-point track on TOP of voxels
        good_tp = (
            np.isfinite(tp_lats)
            & np.isfinite(tp_lons)
        )
        ...
        ax.set_title(
            f"{title_prefix}\n"
            f"RO event {ro_count:02d}: {conid} {prn}"
        )
        ...
        # ============================================================
        # RIGHT: SNR — same as existing code
        # ============================================================

        ax_snr = fig.add_subplot(
            gs[0, 2],
            sharey=ax,
        )
        ...
        if snr_l1 is not None:
            ...
            valid_snr = (
                np.isfinite(snr_l1[:n_snr])
                & np.isfinite(tang[:n_snr])
                & (snr_l1[:n_snr] >= 0)
            )

            ax_snr.plot(
                snr_l1[:n_snr][valid_snr],
                tang[:n_snr][valid_snr],
                "-",
                linewidth=2,
                label="L1 SNR",
            )
        ...
        if snr_l2 is not None:
            ...
            valid_snr = (
                np.isfinite(snr_l2[:n_snr])
                & np.isfinite(tang[:n_snr])
                & (snr_l2[:n_snr] >= 0)
            )

            ax_snr.plot(
                snr_l2[:n_snr][valid_snr],
                tang[:n_snr][valid_snr],
                "--",
                linewidth=2,
                label="L2 SNR",
        ...
        ax_snr.set_xlabel("SNR (V/V)")
        ax_snr.set_title("Signal SNR")
        ax_snr.grid(True, alpha=0.3)
        ax_snr.legend()

        # Remove duplicated y tick labels on right
        plt.setp(
            ax_snr.get_yticklabels(),
            visible=False,
        )
        ...
        filename = (
            f"{title_prefix}_"
            f"RO_{ro_count:02d}_{conid}_{prn}_"
            f"TEC_truth_prior_post.png"
        )
...
def plot_ekf_parameter_std(
    ...
    """
    2x4 maps showing horizontal standard deviation
    for each of the 8 EKF parameters.

    Parameters
    ----------
    state : {"prior", "posterior"}
        Which covariance matrix to plot.

    Each parameter uses its OWN color scale:
        vmin = minimum STD for that parameter
        vmax = maximum STD for that parameter
    """
    ...
    elif state == "posterior":
        P = np.asarray(
            ekf_result["post_P"],
            dtype=float,
        )
        state_label = "Posterior"

    else:
        raise ValueError(
            "state must be 'prior' or 'posterior'"
        )

    grid_lats = np.asarray(
        ekf_result["grid_lats"],
        dtype=float,
    )

    grid_lons = np.asarray(
        ekf_result["grid_lons"],
        dtype=float,
    )
    ...
    lat0 = np.mean(grid_lats)
    lon0 = np.mean(grid_lons)
    ...
    for k, name in enumerate(PARAM_NAMES):
        ...
        # --------------------------------------------------------
        # Covariance block for this parameter
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # Independent color scale for THIS parameter
        # --------------------------------------------------------
        ...
        if finite_sigma.size == 0:
            ...
            vmin = float(
                np.nanmin(finite_sigma)
            )

            vmax = float(
                np.nanmax(finite_sigma)
            )

            # Avoid identical colorbar limits
            if vmax <= vmin:
                delta = max(
                    abs(vmin) * 1e-6,
                    1e-12,
        ...
        print(
            f"  [EKF {state_label} STD] "
            f"{name:<12s} "
            f"min={np.nanmin(sigma):.6g}  "
            f"mean={np.nanmean(sigma):.6g}  "
            f"max={np.nanmax(sigma):.6g}"
        )
    ...
    print(
        f"  Saved EKF {state_label} parameter STD → "
        f"{save_path}"
    )

def plot_ekf_posterior_horizontal_covariance(
    ekf_result,
    save_path,
    title="",
):
    """
    2x4 maps: posterior horizontal correlation for all 8 EKF parameters.
    Correlation is relative to the centre/reference grid point.
    """

    P = np.asarray(ekf_result["post_P"], dtype=float)
    ...
    # Reference = grid point nearest geographical centre
    lat0 = np.mean(grid_lats)
    lon0 = np.mean(grid_lons)
    ...
    proj = ccrs.Orthographic(
        central_longitude=float(grid_lons[ref_idx]),
        central_latitude=float(grid_lats[ref_idx]),
    )
    ...
    cbar.set_label("Posterior horizontal correlation")
...
def plot_ekf_initial_parameter_std(
    ekf_result,
    save_path,
    title="",
):
    """
    Plot initial EKF parameter standard deviation.

    For each of the 8 parameters:
        sigma = sqrt(mean of prior variance over all horizontal voxels))

    Uses the diagonal of ekf_result["prior_P"].
    """

    P = np.asarray(ekf_result["prior_P"], dtype=float)
    ...
    for k, name in enumerate(PARAM_NAMES):
        ...
        # Block corresponding to this parameter
        sl = slice(
            k * n_geo,
            (k + 1) * n_geo,
        )
        ...
        # Variance at every horizontal voxel
        variance = np.maximum(
            np.diag(Pk),
            0.0,
        )

        # One representative STD for this parameter:
        # sqrt(mean variance across all voxels))
        sigma = np.sqrt(
            np.mean(variance)
        )

        param_std.append(sigma)
    ...
    # Put numerical sigma above each bar
    for i, sigma in enumerate(param_std):
        ax.text(
            i,
            sigma,
            f"{sigma:.3g}",
    ...
    print(
        f"  Saved EKF initial parameter STD → "
        f"{save_path}"
    )


def plot_ekf_posterior_parameter_std(
    ekf_result,
    save_path,
    title="",
):
    """
    Plot prior and posterior standard deviation for all 8 EKF parameters.

    Layout:
        rows 1-2 : PRIOR STD for parameters 0-7
        rows 3-4 : POSTERIOR STD for parameters 0-7

    For each parameter and horizontal voxel:

        sigma_prior(g) = sqrt(P_prior[g,g])
        sigma_post(g)  = sqrt(P_post[g,g])

    Each parameter uses the SAME color scale for its prior and posterior maps,
    so uncertainty reduction can be compared directly.
    """
    ...
    # ------------------------------------------------------------
    # Same map centre as existing covariance plot
    # ------------------------------------------------------------
    ...
    for k, name in enumerate(PARAM_NAMES):
        ...
        Pk_prior = P_prior[sl, sl]
        Pk_post  = P_post[sl, sl]
        ...
        # --------------------------------------------------------
        # SAME color scale for prior/posterior of this parameter
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # One shared colorbar for prior + posterior
        # of THIS parameter
        # --------------------------------------------------------
        ...
        prior_mean_std = float(
            np.nanmean(prior_sigma)
        )

        post_mean_std = float(
            np.nanmean(post_sigma)
        )
        ...
        print(
            f"  [EKF STD] "
            f"{name:<12s} "
            f"prior mean={prior_mean_std:.6g}  "
            f"post mean={post_mean_std:.6g}  "
    ...
    print(
        f"  Saved EKF prior/posterior parameter STD → "
        f"{save_path}"
    )
...
def save_iri_vs_8param_profiles_csv(
    iri_edp: np.ndarray,
    ...
    """
    Save the real IRI Ne profile and the Ne profile reconstructed from the
    fitted 8-parameter state for every horizontal voxel.

    Output format is long-form:
        voxel_idx, lat, lon, altitude_km,
        iri_ne_m3, fit8_ne_m3,
        log10_NmF2, hmF2, H0, gamma, B0, B1, log10_NmE, hmE
    """

    iri_edp = np.asarray(iri_edp, dtype=float)
    mean_state = np.asarray(mean_state, dtype=float)
    alt_grid = np.asarray(alt_grid, dtype=float)

    n_alt, n_geo = iri_edp.shape

    if mean_state.shape != (N_STATE, n_geo):
        raise ValueError(
            f"mean_state shape {mean_state.shape}; "
            f"expected ({N_STATE}, {n_geo})"
        )

    # ------------------------------------------------------------
    # Convert mixed EKF state to the linear-density convention
    # expected by _ne_profile_ensemble()
    #
    # state:
    # [log10(NmF2), hmF2, H0, gamma,
    #  B0, B1, log10(NmE), hmE]
    # ------------------------------------------------------------
    params_lin = mean_state.copy()

    params_lin[0, :] = 10.0 ** params_lin[0, :]
    params_lin[6, :] = 10.0 ** params_lin[6, :]

    # Reconstruct Ne(h) from the 8-parameter model at every voxel
    fit8_edp = _ne_profile_ensemble(
        alt_grid,
        params_lin,
    )
    ...
    for g in range(n_geo):
        ...
        for iz, alt in enumerate(alt_grid):

            rows.append({
                "voxel_idx": g,
                "lat_deg": float(grid_lats[g]),
                "lon_deg": float(grid_lons[g]),
                "altitude_km": float(alt),

                "iri_ne_m3": float(iri_edp[iz, g]),
                "fit8_ne_m3": float(fit8_edp[iz, g]),

                "log10_NmF2": float(p[0]),
                "hmF2_km": float(p[1]),
                "H0_km": float(p[2]),
                "gamma": float(p[3]),
                "B0_km": float(p[4]),
    ...
    print(
        f"  [IRI-8P] Saved IRI vs 8-param profiles → "
        f"{save_path}"
    )
...
def _run_parametric_ekf(
    res_kf: dict,
    alt_grid: np.ndarray,
    save_dir: str = "./Data/voxels",
    edps: "list[dict] | None" = None,
    use_esr_cov_tuning: bool = False,
    esr_cov_weight: float = 1.0,
    group_key: str = "group",
    n_members: int = 200,
    ...
    try:
        ...
        # ============================================================
        # EKF prior is fitted from the KF prior Ne
        # ============================================================

        # kf_prior_edp = np.asarray(
        #     prior_edp,
        #     dtype=float,
        # ).copy()

        # if kf_prior_edp.shape != (len(alt_grid), n_geo):
        #     raise ValueError(
        #         "KF prior shape incompatible with EKF grid: "
        #         f"{kf_prior_edp.shape} vs "
        #         f"({len(alt_grid)}, {n_geo})"
        #     )

        # print(
        #     f"  [EKF] Fitting 8-parameter prior directly to "
        #     f"KF Ne prior at {n_geo} horizontal grid points ..."
        # )
        ...
        #     mean_state[:, g] = _fit_iri_params(
        #         kf_prior_edp[:, g],
        #         alt_grid,
        #     )

        # Reference field for diagnostics
        # iri_edp = kf_prior_edp.copy()
        # ============================================================
        # EKF prior must be derived from EXACTLY the same Ne prior
        # already used by the gridded KF.
        # ============================================================

        # kf_prior_edp = np.asarray(prior_edp, dtype=float)

        # print(
        #     f"  [EKF] Fitting 8-parameter state to the KF prior "
        #     f"at {n_geo} grid points ..."
        # )
        ...
        # # This is now the reference IRI/KF prior used by BOTH filters.
        # iri_edp = kf_prior_edp.copy()
    ...
    iri_fit_csv_dir = Path(save_dir) / "IRI_8param_profiles"
    iri_fit_csv_dir.mkdir(
        parents=True,
        exist_ok=True,
    )

    save_iri_vs_8param_profiles_csv(
        iri_edp=iri_edp,
        ...
        save_path=(
            iri_fit_csv_dir
            / f"{group_key}_IRI_vs_8param.csv"
        ),
    )
    """
    # ===== this is the block Autin used for covariance matrix =====
    ...
    # ============================================================
    # IRI covariance — true prior
    # ============================================================

    P_b_prior, C_s = _covariance_from_edp_samples(
        eds_occ,
        alt_grid,
    )

    # Weaken horizontal off-diagonal correlations
    spatial_corr_strength = 0.5   # retain 50% of original correlation

    C_s = (
        spatial_corr_strength * C_s
        + (1.0 - spatial_corr_strength) * np.eye(C_s.shape[0])
    )

    P_b_prior = np.asarray(
        P_b_prior,
        dtype=float,
    )

    if prior_scale != 1.0:
        P_b_prior *= float(prior_scale)


    # ============================================================
    # Show original IRI-generated parameter correlations
    # BEFORE any manual tuning
    # ============================================================

    print_parameter_correlations(
        P_b_prior,
        title="Original IRI-generated prior parameter correlations",
    )


    # ============================================================
    # Optional manual cross-parameter correlation tuning
    # ============================================================

    if EKF_USE_MANUAL_PARAM_CORR:

        print(
            "  [EKF covariance] "
            "Using manually defined parameter correlations."
        )

        P_b_prior = tune_all_parameter_covariances(
            P_b_prior,
            EKF_PARAM_CORR_TUNING,
        )
        ...
        print(
            "  [EKF covariance] "
            "Using default IRI-generated parameter covariance."
        )
    ...
    if n_geo > 1:
        model_state.generate_ensemble_spatial(
            mean_state,
            P_b_prior,
            C_s,
            n_members=n_members,
    ...
    # ============================================================
    # error covariance for EKF UPDATE
    # ============================================================

    esr_cov_diag = None
    update_ensemble = None

    if use_esr_cov_tuning and edps is not None:

        esr_params, esr_edp = _fit_esr_8params(
            edps=edps,
            t_centre=t_centre,
            alt_grid=alt_grid,
        )

        if esr_params is not None:

            P_b_update, esr_cov_diag = _tune_param_covariance_with_esr(
                P_b=P_b_prior,
                mean_state=mean_state,
                grid_lats=grid_lats,
                grid_lons=grid_lons,
                esr_params=esr_params,
                weight=esr_cov_weight,
            )

            update_state = IonosphericState(
                n_grid_points=n_geo,
                n_members=n_members,
            )

            if n_geo > 1:
                update_state.generate_ensemble_spatial(
                    mean_state,
                    P_b_update,
                    C_s,
                    n_members=n_members,
                ...
                update_state.generate_ensemble(
                    mean_state,
                    P_b_update,
                    n_members=n_members,
                )

            update_ensemble = update_state.ensemble.copy()

            print(
                f"  [ESR-COV] Update covariance prepared "
                f"(weight={esr_cov_weight:.3f})"
            )
    ...
    for i, cl in enumerate(clean_list):
        ...
        arc_truth_list.append(dict(
            ...
            conid     = str(leo_id),
            prn_id    = str(prn_id),
            tec_type  = str(cl.get("tec_type", "absolute")),
            snr_l1    = (
                np.asarray(cl["snr_l1"], dtype=float)
                if cl.get("snr_l1") is not None
                else None
            ),

            snr_l2    = (
                np.asarray(cl["snr_l2"], dtype=float)
                if cl.get("snr_l2") is not None
                else None
            ),
        ))

    print(f"  [EKF] Running EKF_Param on {len(arc_truth_list)} arc(s) …")
    H_voxel = res_kf["H_voxel_sparse"]

    if H_voxel.shape[0] != sum(len(a["tec_truth"]) for a in arc_truth_list):
        raise ValueError(
            f"H_voxel has {H_voxel.shape[0]} rows, but EKF has "
            f"{sum(len(a['tec_truth']) for a in arc_truth_list)} rays."
        )
    ...
    ekf_result = EKF_Param(
        ...
        update_ensemble=update_ensemble,
        H_voxel=H_voxel,
        sigma_obs=sigma_obs,
        max_update_rays=max_update_rays,
        alpha=alpha,
        tol=tol,
        max_iter=max_iter,
        tec_rmse_tol=tec_rmse_tol,
        adapt_alpha=adapt_alpha,
        alpha_max=alpha_max,
        step_clip=step_clip,
        jacobian_analytical=True,
        prior_mean=mean_state,
        return_diagnostics=return_diagnostics,
        free_params=free_params,
        param_stages=param_stages,
    )


    # Save ESR covariance-tuning diagnostics with the EKF result
    ekf_result["esr_cov_tuning"] = esr_cov_diag
    ekf_result["esr_cov_tuning_enabled"] = bool(use_esr_cov_tuning)
    ekf_result["esr_cov_weight"] = float(esr_cov_weight)


    # Output directories must exist BEFORE TEC plotting
    diag_dir = Path(save_dir) / "prior_diagnostics"
    diag_dir.mkdir(parents=True, exist_ok=True)

    voxel_dir = Path(save_dir) / "voxel_output"
    voxel_dir.mkdir(parents=True, exist_ok=True)

    tec_plot_dir = voxel_dir / "TEC_prior_arc_comparison"
    tec_plot_dir.mkdir(parents=True, exist_ok=True)

    # ============================================================
    # EKF err covariance / TEC diagnostics
    # ============================================================


    plot_ekf_posterior_horizontal_covariance(
        ekf_result,
        save_path=str(
            diag_dir / f"{group_key}_ekf_posterior_horizontal_covariance.png"
        ),
        title=f"{group_key} — EKF posterior horizontal correlation",
    )

    # ============================================================
    # EKF parameter standard-deviation diagnostics
    # ============================================================

    plot_ekf_initial_parameter_std(
        ekf_result,
        save_path=str(
            diag_dir
            / f"{group_key}_ekf_initial_parameter_std.png"
        ),
        title=(
            f"{group_key} — "
            "EKF Initial Parameter Standard Deviation"
        ),
    )


    # plot_ekf_posterior_parameter_std(
    #     ekf_result,
    #     save_path=str(
    #         diag_dir
    #         / f"{group_key}_ekf_posterior_parameter_std.png"
    #     ),
    #     title=(
    #         f"{group_key} — "
    #         "EKF Posterior Parameter Standard Deviation"
    #     ),
    # )
    ...
    plot_ekf_parameter_std(
        ekf_result,
        save_path=str(
            diag_dir
            / f"{group_key}_ekf_prior_parameter_std.png"
        ),
        state="prior",
        title=(
            f"{group_key} — "
            "EKF Prior Parameter Standard Deviation"
        ),
    )


    # ============================================================
    # POSTERIOR parameter STD
    # ============================================================
    ...
    plot_initial_ekf_error_covariance(
        ekf_result,
        save_path=str(
            diag_dir / f"{group_key}_ekf_initial_error_covariance.png"
        ),
        title=f"{group_key} — EKF initial error covariance",
    )

    # ============================================================
    # Correct KF prior topside Ne BEFORE TEC plotting
    # ============================================================

    kf_prior = np.asarray(prior_edp, dtype=float).copy()
    ekf_prior = np.asarray(ekf_result["prior_edp"], dtype=float)

    

    for g in range(kf_prior.shape[1]):
        kf_prior[:, g] = fill_kf_topside_using_ekf_shape(
            kf_prior[:, g],
            ekf_prior[:, g],
            alt_grid,
            floor_ne=1e8,
    ...
    res_kf["prior_edp_3d"] = kf_prior

    # # ============================================================
    # # Recompute KF TEC from corrected Ne
    # # voxel-Ne contribution only
    # # ============================================================

    # H_voxel = res_kf["H_voxel_sparse"]

    # kf_state_corrected = kf_prior.reshape(-1)

    # kf_prior_tec_corrected_all = np.asarray(
    #     H_voxel @ kf_state_corrected
    # ).ravel()

    # # ============================================================
    # # Rebuild KF per-arc prior TEC slices from corrected Ne
    # # ============================================================

    # kf_tec_slices = res_kf.get(
    #     "joint_tec_slices",
    #     res_kf.get("tec_slices", [])
    # )

    # expected_n_rays = sum(
    #     len(np.asarray(sl.get("prior_tec", [])).ravel())
    #     for sl in kf_tec_slices
    # )

    # if expected_n_rays != len(kf_prior_tec_corrected_all):
    #     raise ValueError(
    #         "[KF topside] H-row / TEC-slice mismatch: "
    #         f"H produced {len(kf_prior_tec_corrected_all)} rays, "
    #         f"but KF TEC slices contain {expected_n_rays} rays."
    #     )

    # offset = 0

    # for i, sl in enumerate(kf_tec_slices):

    #     n = len(np.asarray(sl["prior_tec"]).ravel())

    #     corrected_arc_tec = (
    #         kf_prior_tec_corrected_all[offset:offset + n].copy()
    #     )

    #     # Keep original for diagnostic purposes
    #     sl["prior_tec_original"] = np.asarray(
    #         sl["prior_tec"], dtype=float
    #     ).copy()

    #     # THIS is now the corrected TEC
    #     sl["prior_tec"] = corrected_arc_tec

    #     offset += n

    # print(
    #     f"[KF topside] Updated {len(kf_tec_slices)} KF TEC slices "
    #     f"with corrected-Ne TEC ({offset} rays total)."
    # )

    # ============================================================
    # Recompute KF + EKF PRIOR and POSTERIOR TEC using EXACTLY
    # the SAME KF observation operator H_voxel.
    #
    # Therefore all four TEC curves use the SAME:
    #   - ray geometry
    #   - finite ray segmentation
    #   - spatial interpolation weights
    #   - TEC integration operator
    #
    # If H_voxel_sparse is the 6-point barycentric operator used
    # by the gridded KF, then ALL FOUR curves below use that exact
    # 6-point barycentric TEC integration.
    # ============================================================

    H_voxel = res_kf["H_voxel_sparse"]

    if not issparse(H_voxel):
        raise TypeError(
            "H_voxel_sparse is expected to be a sparse observation matrix."
        )
    ...
    kf_prior_state = np.asarray(
        kf_prior,
        dtype=float,
    ).reshape(-1)

    ekf_prior_state = np.asarray(
        ekf_prior,
        dtype=float,
    ).reshape(-1)
    ...
    kf_post = np.asarray(
        res_kf["joint_post_edp_3d"],
        dtype=float,
    )

    ekf_post = np.asarray(
        ekf_result["posterior_edp"],
        dtype=float,
    )


    # ------------------------------------------------------------
    # Make sure prior/posterior grids are identical in dimensions
    # ------------------------------------------------------------

    if kf_post.shape != kf_prior.shape:
        raise ValueError(
            "KF posterior/prior shape mismatch: "
            f"prior={kf_prior.shape}, "
            f"posterior={kf_post.shape}"
        )

    if ekf_post.shape != ekf_prior.shape:
        raise ValueError(
            "EKF posterior/prior shape mismatch: "
            f"prior={ekf_prior.shape}, "
            f"posterior={ekf_post.shape}"
        )


    kf_post_state = kf_post.reshape(-1)
    ekf_post_state = ekf_post.reshape(-1)


    # ============================================================
    # Check that ALL FOUR states are compatible with H_voxel
    # ============================================================

    n_state_H = H_voxel.shape[1]

    for state_name, state_vector in (
        ("KF prior",      kf_prior_state),
        ("EKF prior",     ekf_prior_state),
        ("KF posterior",  kf_post_state),
        ("EKF posterior", ekf_post_state),
    ):

        if state_vector.size != n_state_H:
            raise ValueError(
                f"{state_name} has {state_vector.size} state elements, "
                f"but H_voxel expects {n_state_H}."
            )


    # ============================================================
    # SAME barycentric / ray integration operator for ALL FOUR
    # ============================================================

    kf_prior_tec_all = np.asarray(
        H_voxel @ kf_prior_state
    ).ravel()

    ekf_prior_tec_all = np.asarray(
        H_voxel @ ekf_prior_state
    ).ravel()

    kf_post_tec_all = np.asarray(
        H_voxel @ kf_post_state
    ).ravel()

    ekf_post_tec_all = np.asarray(
        H_voxel @ ekf_post_state
    ).ravel()


    print(
        "\n[KF/EKF TEC common operator]"
    )

    print(
        f"  H_voxel shape       : {H_voxel.shape}"
    )

    print(
        f"  KF prior TEC rays    : {len(kf_prior_tec_all)}"
    )

    print(
        f"  EKF prior TEC rays   : {len(ekf_prior_tec_all)}"
    )

    print(
        f"  KF posterior TEC rays: {len(kf_post_tec_all)}"
    )

    print(
        f"  EKF posterior TEC rays: {len(ekf_post_tec_all)}"
    )

    ekf_native_prior = np.asarray(ekf_result["y_prior_all"]).ravel()
    ekf_commonH_prior = ekf_prior_tec_all

    diff_prior = ekf_commonH_prior - ekf_native_prior

    # print("\n[EKF PRIOR operator comparison]")
    # print("RMSE(commonH - native):",
    #     np.sqrt(np.mean(diff_prior**2)), "TECU")
    # print("mean diff:", np.mean(diff_prior), "TECU")
    # print("max diff :", np.max(np.abs(diff_prior)), "TECU")

    ekf_native = np.asarray(ekf_result["y_post_all"]).ravel()
    ekf_commonH = ekf_post_tec_all

    diff = ekf_commonH - ekf_native

    # print("\n[EKF operator comparison]")
    # print("native rays :", len(ekf_native))
    # print("commonH rays:", len(ekf_commonH))
    # print("RMSE(commonH - native):", np.sqrt(np.mean(diff**2)), "TECU")
    # print("mean diff:", np.mean(diff), "TECU")
    # print("max diff :", np.max(np.abs(diff)), "TECU")



    # ============================================================
    # Get KF and EKF per-arc TEC slices
    # ============================================================
    ...
    ekf_tec_slices = ekf_result["tec_slices"]


    # ============================================================
    # Make sure KF / EKF contain same number of arcs
    # ============================================================

    if len(kf_tec_slices) != len(ekf_tec_slices):
        raise ValueError(
            "KF/EKF number-of-arcs mismatch: "
            f"KF={len(kf_tec_slices)}, "
            f"EKF={len(ekf_tec_slices)}"
        )


    # ============================================================
    # Total number of rays represented by the per-arc slices
    # ============================================================
    ...
    # ============================================================
    # Check H_voxel output length
    # ============================================================

    for name, tec_array in (
        ("KF prior",      kf_prior_tec_all),
        ("EKF prior",     ekf_prior_tec_all),
        ("KF posterior",  kf_post_tec_all),
        ("EKF posterior", ekf_post_tec_all),
    ):

        if len(tec_array) != expected_n_rays:
            raise ValueError(
                f"{name} common-H TEC contains "
                f"{len(tec_array)} rays, but TEC slices contain "
                f"{expected_n_rays} rays."
            )


    # ============================================================
    # Rebuild KF + EKF per-arc TEC slices.
    #
    # IMPORTANT:
    # We overwrite BOTH prior_tec and post_tec so that the plotting
    # routine automatically uses the common H_voxel results.
    #
    # The original/native values are retained under:
    #
    #     prior_tec_original
    #     post_tec_original
    #
    ...
    for i, (kf_sl, ekf_sl) in enumerate(
        zip(kf_tec_slices, ekf_tec_slices)
    ):

        # --------------------------------------------------------
        # Number of rays in this KF arc
        # --------------------------------------------------------
        ...
        # --------------------------------------------------------
        # Number of rays in corresponding EKF arc
        # --------------------------------------------------------

        n_ekf = len(
            np.asarray(
                ekf_sl.get("prior_tec", [])
            ).ravel()
        )
        ...
        if n_kf != n_ekf:
            raise ValueError(
                f"Arc {i}: KF has {n_kf} rays "
                f"but EKF has {n_ekf} rays."
            )
        ...
        # ========================================================
        # Save ORIGINAL / native filter TEC before replacing it
        # ========================================================

        kf_sl["prior_tec_original"] = np.asarray(
            kf_sl.get("prior_tec", []),
            dtype=float,
        ).copy()

        ekf_sl["prior_tec_original"] = np.asarray(
            ekf_sl.get("prior_tec", []),
            dtype=float,
        ).copy()


        if "post_tec" in kf_sl:

            kf_sl["post_tec_original"] = np.asarray(
                kf_sl["post_tec"],
                dtype=float,
            ).copy()


        if "post_tec" in ekf_sl:

            ekf_sl["post_tec_original"] = np.asarray(
                ekf_sl["post_tec"],
                dtype=float,
            ).copy()
    ...
    # ============================================================
    # Final ray-count check
    # ============================================================

    if offset != expected_n_rays:

        raise ValueError(
            "Final common-H TEC slicing mismatch: "
            f"used {offset} rays, "
            f"expected {expected_n_rays}."
        )


    print(
        f"[KF/EKF TEC] Updated "
        f"{len(kf_tec_slices)} arcs using SAME H_voxel "
        f"for KF prior, EKF prior, KF posterior, EKF posterior "
        f"({offset} total rays)."
    )

    # ============================================================
    # Recompute KF / EKF TEC RMSE using the SAME H_voxel TEC
    # ============================================================

    def _tec_rmse_from_slices(slices, truth_key, model_key):
        obs_all = []
        model_all = []
        ...
        for sl in slices:
            ...
            if valid.any():
                obs_all.append(obs[valid])
                model_all.append(model[valid])
    ...
    # KF uses "measured"
    kf_prior_rmse_commonH = _tec_rmse_from_slices(
        kf_tec_slices,
        "measured",
        "prior_tec",
    )

    kf_post_rmse_commonH = _tec_rmse_from_slices(
        kf_tec_slices,
        "measured",
        "post_tec",
    )
    ...
    # EKF uses "tec_truth"
    ekf_prior_rmse_commonH = _tec_rmse_from_slices(
        ekf_tec_slices,
        "tec_truth",
        "prior_tec",
    )

    ekf_post_rmse_commonH = _tec_rmse_from_slices(
        ekf_tec_slices,
        "tec_truth",
        "post_tec",
    )

    res_kf["prior_tec_rmse"] = kf_prior_rmse_commonH
    res_kf["post_tec_rmse"] = kf_post_rmse_commonH
    res_kf["joint_post_tec_rmse"] = kf_post_rmse_commonH

    # ============================================================
    # Plot AFTER common-H TEC has replaced all four model curves
    # ============================================================

    plot_ekf_ro_tec_truth_prior_post(
        arc_truth_list,
        ekf_result,
        res_kf,
        save_dir=diag_dir / "RO_TEC_truth_prior_post",
        title_prefix=group_key,
    )


    # ============================================================
    # Plot every TEC arc:
    # left  = tangent-point geographic track
    # right = KF/EKF prior TEC versus tangent altitude
    # ============================================================
    TRO_LAT = 69.583
    TRO_LON = 19.21
    ...
    ro_indices = [
        i for i, cl in enumerate(clean_list)
        if cl.get("tec_type", "absolute") != "relative"
        and len(np.asarray(cl.get("tangent_km", []))) > 1
    ]
    ...
    print("Saving KF & EKF prior TEC Comparison Plots")
    for arc_index in ro_indices:

        arc = arc_truth_list[arc_index]
        kf_slice = kf_tec_slices[arc_index]
        ekf_slice = ekf_tec_slices[arc_index]

        kf_tang = _safe_array(
            kf_slice.get("tangent_km", [])
        )

        ekf_tang = _safe_array(
            ekf_slice.get(
                "tang_km",
                ekf_slice.get("tangent_km", [])
            )
        )

        arc_tang = _safe_array(
            arc.get("tang_km", [])
        )

        # print(
        #     f"\n[TEC arc check] arc={arc_index:03d} "
        #     f"{arc.get('conid')}_{arc.get('prn_id')}"
        # )

        # print(
        #     "  ray counts:"
        #     f" geometry={len(arc_tang)},"
        #     f" KF={len(kf_slice.get('prior_tec', []))},"
        #     f" EKF={len(ekf_slice.get('prior_tec', []))}"
        # )

        # if (
        #     kf_tang.size == arc_tang.size
        #     and kf_tang.size > 0
        # ):
        #     print(
        #         "  KF tangent-alt max difference:",
        #         np.nanmax(
        #             np.abs(kf_tang - arc_tang)
        #         ),
        #         "km"
        #     )

        # if (
        #     ekf_tang.size == arc_tang.size
        #     and ekf_tang.size > 0
        # ):
        #     print(
        #         "  EKF tangent-alt max difference:",
        #         np.nanmax(
        #             np.abs(ekf_tang - arc_tang)
        #         ),
        #         "km"
        #     )
        # --------------------------------------------------------
        # Geographic and altitude information
        # --------------------------------------------------------

        tangent_lat = _safe_array(
            arc.get("tp_lats", [])
        )

        tangent_lon = _safe_array(
            arc.get("tp_lons", [])
        )

        tangent_alt_km = _safe_array(
            arc.get(
                "tang_km",
                ekf_slice.get("tang_km", []),
            )
        )
        ...
        kf_prior_tec = _safe_array(
            kf_slice.get("prior_tec", [])
        )

        ekf_prior_tec = _safe_array(
            ekf_slice.get("prior_tec", [])
        )

        # Use only the common number of matched rays
        n_ray = min(
            tangent_lat.size,
            tangent_lon.size,
            tangent_alt_km.size,
        ...
        if n_ray < 2:
            print(
                f"Skipping TEC arc {arc_index}: "
                f"only {n_ray} matched rays."
            )
            continue

        tangent_lat = tangent_lat[:n_ray]
        tangent_lon = tangent_lon[:n_ray]
        tangent_alt_km = tangent_alt_km[:n_ray]
        observed_tec = observed_tec[:n_ray]
        kf_prior_tec = kf_prior_tec[:n_ray]
        ekf_prior_tec = ekf_prior_tec[:n_ray]

        valid = (
            np.isfinite(tangent_lat)
            & np.isfinite(tangent_lon)
            & np.isfinite(tangent_alt_km)
            & np.isfinite(kf_prior_tec)
            & np.isfinite(ekf_prior_tec)
        )

        if valid.sum() < 2:
            print(
                f"Skipping TEC arc {arc_index}: "
                "insufficient finite matched rays."
            )
            continue

        tangent_lat = tangent_lat[valid]
        tangent_lon = tangent_lon[valid]
        tangent_alt_km = tangent_alt_km[valid]
        kf_prior_tec = kf_prior_tec[valid]
        ekf_prior_tec = ekf_prior_tec[valid]
        observed_tec = observed_tec[valid]

        # Sort the right panel by tangent altitude
        altitude_order = np.argsort(tangent_alt_km)

        alt_plot = tangent_alt_km[altitude_order]
        kf_plot = kf_prior_tec[altitude_order]
        ekf_plot = ekf_prior_tec[altitude_order]
        obs_plot = observed_tec[altitude_order]

        # --------------------------------------------------------
        # RMSE against the actual observed full-ray TEC
        # --------------------------------------------------------

        valid_obs = np.isfinite(obs_plot)
        ...
        if valid_obs.any():
            ...
            kf_prior_rmse = np.nan
            ekf_prior_rmse = np.nan

        # --------------------------------------------------------
        # Arc label
        # --------------------------------------------------------

        conid = arc.get("conid", "unknown")
        prn_id = arc.get("prn_id", "unknown")
        arc_label = f"{conid}_{prn_id}"

        tec_type = arc.get("tec_type", "unknown")
        ...
        # Connect the tangent points to show the arc direction
        ax_map.plot(
            tangent_lon,
            tangent_lat,
            linewidth=1.2,
        ...
        tangent_scatter = ax_map.scatter(
            tangent_lon,
            tangent_lat,
            c=tangent_alt_km,
            s=45,
        ...
        # First and final tangent points
        ax_map.scatter(
            tangent_lon[0],
            tangent_lat[0],
            marker="o",
        ...
        map_colorbar.set_label(
            "RO tangent altitude (km)",
            fontsize=16,
        )

        map_colorbar.ax.tick_params(
            labelsize=14
        )

        ax_map.set_title(
            "RO Tangent-Point Track",
            fontsize=20,
        )
        ...
        # ========================================================
        # Right panel: full-ray prior TEC versus tangent altitude
        # ========================================================

        ax_tec = fig.add_subplot(
            1,
            2,
            2,
        )
        ...
        ax_tec.plot(
            ...
            label=(
                "KF prior TEC\n"
                f"RMSE vs. observation = "
                f"{kf_prior_rmse:.2f} TECU"
            ),
        )
        ...
        ax_tec.plot(
            ...
            label=(
                "EKF prior TEC\n"
                f"RMSE vs. observation = "
                f"{ekf_prior_rmse:.2f} TECU"
            ),
        )

        ax_tec.set_xlabel(
            "Prior slant TEC (TECU)",
            fontsize=18,
        )
        ...
        fig.suptitle(
            f"{group_key} | Arc {arc_index:03d}: {arc_label}\n"
            f"TEC type: {tec_type} | Number of rays: {len(alt_plot)}",
            fontsize=21,
        )
        ...
        output_name = (
            f"{_safe_filename(group_key)}_"
            f"arc_{arc_index:03d}_"
            f"{_safe_filename(arc_label)}_"
            "KF_EKF_prior_TEC.png"
        )

        output_path = (
            tec_plot_dir / output_name
        )
    ...
    ekf_prior = np.asarray(ekf_result["prior_edp"], dtype=float)
    # ============================================================
    # Correct KF artificial topside floor using EKF profile shape
    # ============================================================

    kf_prior = kf_prior.copy()
    ...
    # Make the corrected KF prior the one used downstream
    res_kf["prior_edp_3d"] = kf_prior

    print(
        "[KF topside] Corrected KF prior profiles:",
        kf_prior.shape
    )
    # ============================================================
    # Save KF/EKF prior Ne at every voxel and altitude
    # ============================================================

    if kf_prior.shape != ekf_prior.shape:
        raise ValueError(
            "Cannot save direct KF/EKF voxel comparison: "
            f"KF={kf_prior.shape}, EKF={ekf_prior.shape}"
        )

    n_alt, n_geo = kf_prior.shape

    # DataFrame ordering:
    # geo0: all altitudes
    # geo1: all altitudes
    # ...
    voxel_compare_df = pd.DataFrame({
        "geo_index": np.repeat(
            np.arange(n_geo),
            n_alt,
        ),
        ...
        "kf_prior_ne_m3": (
            kf_prior.T.reshape(-1)
        ),

        "ekf_prior_ne_m3": (
            ekf_prior.T.reshape(-1)
        ),
    })

    voxel_compare_df["ekf_minus_kf_ne_m3"] = (
        voxel_compare_df["ekf_prior_ne_m3"]
        - voxel_compare_df["kf_prior_ne_m3"]
    )

    voxel_compare_df["ekf_over_kf"] = np.divide(
        voxel_compare_df["ekf_prior_ne_m3"],
        voxel_compare_df["kf_prior_ne_m3"],
        out=np.full(
            len(voxel_compare_df),
            np.nan,
            dtype=float,
        ),
        where=(
            voxel_compare_df["kf_prior_ne_m3"].to_numpy()
            > 0
        ),
    )

    voxel_compare_df["log10_kf_ne"] = np.log10(
        np.maximum(
            voxel_compare_df["kf_prior_ne_m3"],
            1.0,
        )
    ...
    voxel_compare_df["log10_ekf_ne"] = np.log10(
        np.maximum(
            voxel_compare_df["ekf_prior_ne_m3"],
            1.0,
        )
    ...
    voxel_compare_path = (
        voxel_dir
        / f"{group_key}_KF_EKF_prior_Ne_voxel_comparison.csv"
    )

    voxel_compare_df.to_csv(
        voxel_compare_path,
        index=False,
    )

    print(
        "Saved KF/EKF prior Ne voxel comparison → "
        f"{voxel_compare_path}"
    )

    altitude_diagnostics = []
    ...
    for ia, alt in enumerate(alt_grid):
        ...
        altitude_diagnostics.append({
            "alt_km": float(alt),

            "kf_median_ne_m3":
                float(np.nanmedian(kf_a[valid])),

            "ekf_median_ne_m3":
                float(np.nanmedian(ekf_a[valid])),

            "rmse_ne_m3":
                float(np.sqrt(np.nanmean(diff**2))),

            "median_ekf_over_kf":
                float(np.nanmedian(ratio)),

            "p05_ekf_over_kf":
                float(np.nanpercentile(ratio, 5)),

            "p95_ekf_over_kf":
                float(np.nanpercentile(ratio, 95)),
        })

    alt_diag_df = pd.DataFrame(
        altitude_diagnostics
    )

    alt_diag_path = (
        voxel_dir
        / f"{group_key}_KF_EKF_prior_Ne_by_altitude.csv"
    )
    ...
    print(
        "Saved KF/EKF prior altitude diagnostic → "
        f"{alt_diag_path}"
    )
    ...
    if not issparse(H_voxel):
        raise TypeError("H_voxel_sparse is expected to be a sparse matrix.")

    # State ordering used by H:
    # [altitude 0: all horizontal points,
    #  altitude 1: all horizontal points, ...]
    kf_state = kf_prior.reshape(-1)
    ekf_state = ekf_prior.reshape(-1)

    if H_voxel.shape[1] != kf_state.size:
        raise ValueError(
            f"H columns={H_voxel.shape[1]}, "
            f"but voxel state size={kf_state.size}"
        )


    # Contribution of voxel j to ray i:
    # H[i,j] * Ne[j] = TECU
    kf_contrib = H_voxel.multiply(kf_state[np.newaxis, :])
    ekf_contrib = H_voxel.multiply(ekf_state[np.newaxis, :])

    # Sum over every observation ray for each voxel
    kf_sum_tecu = np.asarray(kf_contrib.sum(axis=0)).ravel()
    ekf_sum_tecu = np.asarray(ekf_contrib.sum(axis=0)).ravel()

    # Number of rays crossing each voxel
    ray_count = np.asarray(
        (H_voxel != 0).sum(axis=0)
    ).ravel().astype(int)

    # Mean contribution per intersecting ray
    kf_mean_tecu = np.divide(
        kf_sum_tecu,
        ray_count,
        out=np.full_like(kf_sum_tecu, np.nan, dtype=float),
        where=ray_count > 0,
    )

    ekf_mean_tecu = np.divide(
        ekf_sum_tecu,
        ray_count,
        out=np.full_like(ekf_sum_tecu, np.nan, dtype=float),
        where=ray_count > 0,
    )

    # Convert state ordering into coordinates
    alt_index = np.repeat(
        np.arange(len(alt_grid)),
        n_geo,
    )
    ...
    tec_voxel_df = pd.DataFrame({
        "voxel_state_index": np.arange(kf_state.size),
        "lat": grid_lats[geo_index],
        "lon": grid_lons[geo_index],
        "alt_km": alt_grid[alt_index],
        "ray_count": ray_count,

        "kf_prior_ne_m3": kf_state,
        "ekf_prior_ne_m3": ekf_state,

        # Sum across all rays; depends on the number of observations
        "kf_prior_tec_sum_tecu": kf_sum_tecu,
        "ekf_prior_tec_sum_tecu": ekf_sum_tecu,

        # Better for comparing voxels with different ray coverage
        "kf_prior_tec_mean_per_ray_tecu": kf_mean_tecu,
        "ekf_prior_tec_mean_per_ray_tecu": ekf_mean_tecu,
    })

    tec_voxel_path = (
        voxel_dir
        / f"{group_key}_KF_EKF_prior_TEC_voxel_contributions.csv"
    )

    tec_voxel_df.to_csv(tec_voxel_path, index=False)

    print(f"Saved prior TEC voxel contributions → {tec_voxel_path}")

    if "voxel_obs_count" in res_kf:
        obs_count = np.asarray(
            res_kf["voxel_obs_count"],
            dtype=int,
        )

    elif "H_voxel_sparse" in res_kf:
        # Number of observation rays intersecting each voxel
        obs_count = np.asarray(
            (res_kf["H_voxel_sparse"] != 0).sum(axis=0)
        ).ravel().astype(int)

        obs_count = obs_count.reshape(
            len(alt_grid),
            n_geo,
        )
        ...
        raise KeyError(
            "Neither voxel_obs_count nor H_voxel_sparse is available."
        )

    obs_count_df = pd.DataFrame({
        "lat": np.repeat(grid_lats, len(alt_grid)),
        "lon": np.repeat(grid_lons, len(alt_grid)),
        "alt_km": np.tile(alt_grid, n_geo),
        "observation_count": obs_count.T.reshape(-1),
    })

    obs_path = (
        voxel_dir
        / f"{group_key}_KF_voxel_observation_counts.csv"
    )

    obs_count_df.to_csv(obs_path, index=False)

    print(f"Saved voxel observation counts → {obs_path}")

    # KF: one row per lat/lon/alt voxel
    kf_voxels = pd.DataFrame({
        "lat": np.repeat(grid_lats, len(alt_grid)),
        "lon": np.repeat(grid_lons, len(alt_grid)),
        "alt_km": np.tile(alt_grid, n_geo),
        "prior_ne_m3": kf_prior.T.reshape(-1),
        "post_ne_m3": np.asarray(
            res_kf["joint_post_edp_3d"], dtype=float
        ).T.reshape(-1),
    })

    kf_path = voxel_dir / f"{group_key}_KF_voxels.csv"
    kf_voxels.to_csv(kf_path, index=False)

    # EKF: one row per horizontal column, with 8 parameters
    ekf_columns = pd.DataFrame({
    ...
    for i, name in enumerate(PARAM_NAMES):
        ekf_columns[f"prior_{name}"] = ekf_result["prior_mean_state"][i, :]
        ekf_columns[f"post_{name}"] = ekf_result["posterior_mean_state"][i, :]

    ekf_path = voxel_dir / f"{group_key}_EKF_columns.csv"
    ekf_columns.to_csv(ekf_path, index=False)

    # EKF reconstructed 3-D Ne field
    ekf_voxels = pd.DataFrame({
        "lat": np.repeat(grid_lats, len(alt_grid)),
        "lon": np.repeat(grid_lons, len(alt_grid)),
        "alt_km": np.tile(alt_grid, n_geo),
        "prior_ne_m3": ekf_prior.T.reshape(-1),
        "post_ne_m3": np.asarray(
            ekf_result["posterior_edp"], dtype=float
        ).T.reshape(-1),
    })

    ekf_ne_path = voxel_dir / f"{group_key}_EKF_reconstructed_voxels.csv"
    ekf_voxels.to_csv(ekf_ne_path, index=False)

    # print(f"Saved KF voxels → {kf_path}")
    # print(f"Saved EKF parameter columns → {ekf_path}")
    # print(f"Saved EKF reconstructed voxels → {ekf_ne_path}")
    ...
    # ============================================================
    # RMSE using the NEW common-H prior TEC
    # ============================================================

    kf_prior_rmse_commonH = _rmse(
        kf_obs,
        kf_prior_tec
    )

    ekf_prior_rmse_commonH = _rmse(
        ekf_obs,
        ekf_prior_tec
    )
    ...
    ekf_result["prior_rmse_original"] = ekf_result.get(
        "prior_rmse",
        np.nan
    )

    ekf_result["prior_rmse"] = ekf_prior_rmse_commonH
    ...
    res_ekf["kf_prior_tec_rmse_commonH"]       = kf_prior_rmse_commonH
    res_ekf["prior_tec_rmse"]      = ekf_prior_rmse_commonH
    res_ekf["post_tec_rmse"]       = ekf_post_rmse_commonH
    res_ekf["joint_post_tec_rmse"] = ekf_post_rmse_commonH
    res_ekf["ekf_converged"]       = ekf_result["converged"]
    res_ekf["ekf_n_iterations"]    = ekf_result["n_iterations"]
    res_ekf["esr_cov_tuning"] = ekf_result.get("esr_cov_tuning")
    res_ekf["esr_cov_tuning_enabled"] = ekf_result.get(
        "esr_cov_tuning_enabled", False)
    res_ekf["esr_cov_weight"] = ekf_result.get(
        "esr_cov_weight", np.nan)
...
def print_parameter_correlations(
    P_b,
    title="IRI-generated parameter correlations",
):
    """
    Print the correlation matrix and all 28 unique pairwise
    correlations from an 8x8 parameter covariance matrix.

    This is useful for copying the current IRI-derived correlation
    values into ekf_param_correlation_config.py.
    """
    ...
    if P_b.shape != (N_STATE, N_STATE):
        raise ValueError(
            f"Expected ({N_STATE}, {N_STATE}) covariance matrix, "
            f"got {P_b.shape}"
        )
    ...
    *windows* is the list of dicts returned by build_minima_windows_for_day()
    ...
    def _process_window_bin(window: dict, bin_count: "int | None") -> None:
        ...
        *bin_count* subsamples window["group_meta"]'s RO occultations down to
            ...
            for _ekf_label, _free in ekf_modes:
                if obs_mode == "igs_only":
                    def _run_igs_ekf(kf_result=kf_result, t_centre=t_centre,
                        ...
                        # )
                        return _run_parametric_ekf(
                            res_kf=res_kf_adapted,
                            alt_grid=ALT_GRID,
                            save_dir=str(group_dirs["igs_only"]),
                            group_key=f"{group_key}_igs_only_{_ekf_label}",

                            edps=edps,
                            use_esr_cov_tuning=EKF_ESR_COV_TUNING,
                            esr_cov_weight=EKF_ESR_COV_WEIGHT,

                            n_members=EKF_N_MEMBERS,
                            sigma_obs=EKF_SIGMA_OBS,
                            max_update_rays=EKF_MAX_RAYS,
                            alpha=EKF_ALPHA,
                            tol=EKF_TOL,
                            max_iter=EKF_MAX_ITER,
                            prior_scale=EKF_PRIOR_SCALE,
                            free_params=_free,
                            adapt_alpha=EKF_ADAPT_ALPHA,
                            alpha_max=EKF_ALPHA_MAX,
                            tec_rmse_tol=EKF_TEC_RMSE_TOL,
                        )
                ...
                def _run_ekf(kf_result=kf_result, obs_mode=obs_mode,
                    ...
                    if res_ekf is not None:
                        kf_result["prior_tec_rmse"] = (
                            res_ekf["kf_prior_tec_rmse_commonH"]
                        )
        ...
        # ============================================================
        # Harmonize KF prior topside Ne with EKF-shaped transition
        # ============================================================

        for obs_mode in OBS_MODES:

            res_kf = group_filter_results[obs_mode].get("gridded_kf")
            res_ekf = group_filter_results[obs_mode].get("parametric_ekf")

            if res_kf is None or res_ekf is None:
                continue

            if (
                res_kf.get("status", "Success") != "Success"
                or res_ekf.get("status", "Success") != "Success"
            ):
                continue
            ...
            ekf_prior = np.asarray(
                res_ekf["prior_edp_3d"],
                dtype=float,
            )

            # Must be same altitude × horizontal-grid shape
            if kf_prior.shape != ekf_prior.shape:
                print(
                    f"  [warn] Cannot apply KF topside correction for "
                    f"{obs_mode}: KF shape={kf_prior.shape}, "
                    f"EKF shape={ekf_prior.shape}"
                )
                continue
            ...
            # IMPORTANT:
            # replace the KF prior that all downstream code sees
            res_kf["prior_edp_3d"] = kf_prior
        ...
        bin_save_dir.mkdir(parents=True, exist_ok=True)

        res_kf = group_filter_results["ro_only"]["gridded_kf"]
        res_ekf = group_filter_results["ro_only"]["parametric_ekf"]

        if res_kf is not None and res_ekf is not None and window_edps:
            plot_overall_rmse_vs_altitude(
                res_kf=res_kf,
                res_ekf=res_ekf,
                measured_ne_profiles=window_edps,
                alt_grid=ALT_GRID,
                save_path=(
                    bin_save_dir
                    / _safe_group_key(group_key)
                    / f"{_safe_group_key(group_key)}_overall_RMSE_vs_altitude.png"
                ),
                tec_bin_km=15.0,
            )
...
def plot_overall_rmse_vs_altitude(
    res_kf,
    res_ekf,
    measured_ne_profiles,
    alt_grid,
    save_path,
    tec_bin_km=10.0,
):
    ...
    # ============================================================
    # 1. TEC RMSE versus RO tangent altitude
    # ============================================================
    def pooled_tec(slices):
        alt, prior_err, post_err = [], [], []

        for sl in slices:
            truth = np.asarray(
                sl.get("measured", sl.get("tec_truth", [])),
                dtype=float,
            )
            prior = np.asarray(sl["prior_tec"], dtype=float)
            post = np.asarray(sl["post_tec"], dtype=float)
            tang = np.asarray(
                sl.get("tangent_km", sl.get("tang_km", [])),
                dtype=float,
            )

            n = min(len(truth), len(prior), len(post), len(tang))
            ...
            alt.extend(tang[:n][valid])
            prior_err.extend((prior[:n][valid] - truth[:n][valid]) ** 2)
            post_err.extend((post[:n][valid] - truth[:n][valid]) ** 2)

        return (
            np.asarray(alt),
            np.asarray(prior_err),
            np.asarray(post_err),
        )

    kf_alt, kf_prior_sq, kf_post_sq = pooled_tec(
        res_kf.get("joint_tec_slices", res_kf["tec_slices"])
    )

    ekf_alt, ekf_prior_sq, ekf_post_sq = pooled_tec(
        res_ekf["tec_slices"]
    )

    edges = np.arange(
        0,
        max(np.nanmax(kf_alt), np.nanmax(ekf_alt)) + tec_bin_km,
        tec_bin_km,
    )
    centers = 0.5 * (edges[:-1] + edges[1:])

    def bin_rmse(alt, sq_error):
        ...
        for i in range(len(centers)):
            ...
            if np.any(mask):
                output[i] = np.sqrt(np.mean(sq_error[mask]))

        return output

    kf_tec_prior = bin_rmse(kf_alt, kf_prior_sq)
    kf_tec_post = bin_rmse(kf_alt, kf_post_sq)
    ekf_tec_prior = bin_rmse(ekf_alt, ekf_prior_sq)
    ekf_tec_post = bin_rmse(ekf_alt, ekf_post_sq)

    # ============================================================
    # 2. Ne RMSE versus altitude
    # ============================================================
    def pooled_ne(result):
        prior = np.asarray(result["prior_edp_3d"], dtype=float)
        post = np.asarray(result["joint_post_edp_3d"], dtype=float)

        geolocation = np.asarray(
            result["eds_occ"].geolocation,
            dtype=float,
        )

        model_lon = geolocation[:, 0]
        model_lat = geolocation[:, 1]

        prior_sq = [[] for _ in alt_grid]
        post_sq = [[] for _ in alt_grid]

        for profile in measured_ne_profiles:
            obs_alt = np.asarray(profile["alt_km"], dtype=float)
            obs_ne = np.asarray(profile["ne_m3"], dtype=float)

            distance = (
                (model_lat - float(profile["lat"])) ** 2
                + (model_lon - float(profile["lon"])) ** 2
            )
            g = int(np.argmin(distance))
            ...
            for iz in range(len(alt_grid)):
                if np.isfinite(obs_interp[iz]):
                    prior_sq[iz].append(
                        (prior[iz, g] - obs_interp[iz]) ** 2
                    )
                    post_sq[iz].append(
                        (post[iz, g] - obs_interp[iz]) ** 2
                    )

        prior_rmse = np.array([
            np.sqrt(np.mean(v)) if v else np.nan
            for v in prior_sq
        ])
    ...
    kf_ne_prior, kf_ne_post = pooled_ne(res_kf)
    ekf_ne_prior, ekf_ne_post = pooled_ne(res_ekf)
    ...
    ax = axes[0]
    ax.plot(kf_tec_prior, centers, "g--", label="KF prior")
    ax.plot(kf_tec_post, centers, "m-", label="KF posterior")
    ax.plot(ekf_tec_prior, centers, "b--", label="EKF prior")
    ax.plot(ekf_tec_post, centers, "r-", label="EKF posterior")
    ax.set_xlabel("TEC RMSE (TECU)")
    ax.set_ylabel("RO Tangent Height (km)")
    n_ro_scans = len(res_kf.get("clean_list", []))
    ax.set_title(
        f"Overall TEC RMSE vs. Tangent Height\n"
        f"Across {n_ro_scans} RO Scans"
    )
    ax.grid(alpha=0.3)
    ax.legend()

    ax = axes[1]
    ax.plot(kf_ne_prior, alt_grid, "g--", label="KF prior")
    ax.plot(kf_ne_post, alt_grid, "m-", label="KF posterior")
    ax.plot(ekf_ne_prior, alt_grid, "b--", label="EKF prior")
    ax.plot(ekf_ne_post, alt_grid, "r-", label="EKF posterior")
    ax.set_xscale("log")
    ax.set_xlabel(r"Ne RMSE (m$^{-3}$)")
    ax.set_ylabel("Altitude (km)")
    ax.set_title(
        f"The Closest Ne RMSE vs. Altitude\n"
        f"with Respect to ISR Ne"
    )
    ax.grid(alpha=0.3)
    ax.legend()
    ...
    print(f"Saved overall TEC/Ne RMSE → {save_path}")
    ...
    for obs_mode, per_filter in filter_results.items():
        for filter_type, result in per_filter.items():
            ...
            for inst_name, edp in window_edps:
                ...
                # + 150 km to extend the comparison up to ~450 km. And this will not over the sat alt that 1e8 Ne cutoff happened
                below_peak = (isr_alt <= prior_hmF2 + 150.0) if np.isfinite(prior_hmF2) \
                             else np.ones(len(isr_alt), dtype=bool)

                # prior_at_isr = np.interp(isr_alt, alt_grid, prior_col) # may need 2 change this in case the extrapolate happens, Pin Aug 4,26
                # post_at_isr  = np.interp(isr_alt, alt_grid, post_col)

                # exclude the exterpolation
                ...
                # valid = (isr_ne > 1e7) & np.isfinite(isr_ne) # below peak is not necessary anymore if KF is using Ne of EKF in the topside ionosphere
                valid = (
                    (isr_ne > 1e7)
                    & np.isfinite(isr_ne)
                ...
                valid_all = ((isr_ne > 1e7) & np.isfinite(isr_ne)
                             & np.isfinite(post_at_isr) & np.isfinite(prior_at_isr))

                # print(
                #     f"ISR RMSE altitude range: "
                #     f"{isr_alt[valid].min():.1f}–"
                #     f"{isr_alt[valid].max():.1f} km, "
                #     f"n={valid.sum()} gates"
                # )
...
def _fit_esr_8params(
    ...
    # ============================================================
    # Find ESR profile closest to window centre
    # ============================================================
    ...
    for e in edps:
        ...
        if (
            ...
            or
            abs(float(e["lon"]) - esr_inst["lon"]) > ISR_SITE_MATCH_DEG
        ):
            continue
    ...
    if not candidates:
        print(f"  [ESR-COV] No ESR profile near {t_centre}")
        return None, None
    ...
    if len(esr_alt) < ISR_MIN_VALID_GATES:
        print("  [ESR-COV] Too few ESR gates")
        return None, None


    # ============================================================
    # 1. F2 peak: ONLY search >= 200 km
    # ============================================================
    f_mask = (
        (esr_alt >= 220.0)
        & (esr_alt <= 600.0)
    ...
    if np.sum(f_mask) < 3:
        print("  [ESR-COV] Insufficient ESR measurements above 200 km")
        return None, None

    f_alt = esr_alt[f_mask]
    f_ne  = esr_ne[f_mask]

    i_f2 = np.argmax(f_ne)

    nm_f2 = float(f_ne[i_f2])
    hm_f2 = float(f_alt[i_f2])


    # ============================================================
    # 2. E-region peak: restrict to approximately 80–160 km
    # ============================================================
    e_mask = (
        (esr_alt >= 80.0)
        & (esr_alt <= 180.0)
    ...
    # ============================================================
    # 3. Initial topside scale H0
    # ============================================================
    top_mask = esr_alt > hm_f2

    if np.sum(top_mask) >= 3:

        alt_top = esr_alt[top_mask]
        ne_top  = esr_ne[top_mask]
        ...
        if len(above) > 0:
            H0 = float(above[-1] - hm_f2)
        else:
            H0 = 60.0

        H0 = np.clip(H0, 20.0, 200.0)
    ...
    if np.sum(bottom_mask) >= 3:
        ...
        alt_bot = esr_alt[bottom_mask]
        ne_bot  = esr_ne[bottom_mask]
        ...
        if len(below) > 0:
            B0 = hm_f2 - float(below[0])
        else:
            B0 = 60.0

        B0 = np.clip(B0, 20.0, 200.0)

    else:
        B0 = 60.0


    # Starting values for harder-to-extract shape parameters
    gamma = 0.5
    B1 = 1.5


    # ============================================================
    # Fix ESR-observed NmF2 and hmF2.
    # Only fit the remaining 6 shape parameters.
    # ============================================================

    log_nm_f2_fixed = np.log10(nm_f2)
    hm_f2_fixed = hm_f2

    # print("\n  [ESR-COV] ESR fixed F2 peak:")
    # print(f"            log10(NmF2) = {log_nm_f2_fixed:.6g}")
    # print(f"            NmF2        = {nm_f2:.6e} m^-3")
    # print(f"            hmF2        = {hm_f2_fixed:.2f} km")


    # Parameters being optimized:
    # [H0, gamma, B0, B1, log10(NmE), hmE]
    x0_fit = np.array([
        H0,
        gamma,
        B0,
    ...
    # ============================================================
    # Fit only over actual ESR altitude range
    # ============================================================
    ...
    fit_alt = esr_alt[fit_mask]
    fit_ne  = esr_ne[fit_mask]


    # ============================================================
    # Bounds for remaining 6 parameters
    # ============================================================
    ...
    # ============================================================
    # Cost function
    # NmF2 and hmF2 NEVER change here
    # ============================================================
    ...
    def _cost(x):
        ...
        ne_model = _ne_profile_ensemble(
            fit_alt,
            params_lin,
        )[:, 0]

        obs_log = np.log10(
            np.maximum(fit_ne, 1.0)
        )

        model_log = np.log10(
            np.maximum(ne_model, 1.0)
        )
    ...
    if result.success or result.fun < _cost(x0_fit):
    ...
    # ============================================================
    # Rebuild complete 8-param ESR state
    # ============================================================

    esr_params = np.array([
        log_nm_f2_fixed,   # ESR truth
        hm_f2_fixed,       # ESR truth
        x_fit[0],          # H0
        x_fit[1],          # gamma
        x_fit[2],          # B0
        x_fit[3],          # B1
    ...
    # print("  [ESR-COV] Final ESR fitted parameters:")
    ...
    # ============================================================
    # Reconstruct fitted ESR profile
    #
    # _ne_profile_ensemble expects:
    #   alts first
    #   LINEAR-density parameter array second
    # ============================================================
    params_lin = np.array([
        10.0 ** esr_params[0],
        esr_params[1],
    ...
    esr_fit_ne = _ne_profile_ensemble(
        alt_grid,
        params_lin,
    )[:, 0]
    ...
    ax.axhline(
        ...
        linestyle=":",
        linewidth=1.5,
        label=f"hmF2 = {esr_params[1]:.1f} km",
    )
    ...
    out_dir = SAVE_DIR / "ESR_8param_fit"
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(
        f"  [ESR-COV] Saved ESR fit plot → {out_path}"
    )
...
def _tune_param_covariance_with_esr(
    ...
    # ============================================================
    # Original parameter standard deviations
    # ============================================================

    std = np.sqrt(
        np.maximum(np.diag(P_b), 1e-20)
    )

    D = np.diag(std)

    # Original correlation matrix
    C_original = P_b / np.outer(std, std)
    ...
    # ============================================================
    # ESR error-direction correlation
    #
    # One ESR sample cannot estimate a statistical covariance.
    # It only tells us the observed direction of parameter errors.
    # ============================================================

    sgn = np.sign(error)

    # zero-error parameter -> don't force a sign
    sgn[sgn == 0.0] = 0.0

    C_esr = np.outer(sgn, sgn)

    # Keep diagonal correlation = 1
    np.fill_diagonal(C_esr, 1.0)
    ...
    diagnostics = {
        ...
        "C_original": C_original.copy(),
        "C_esr": C_esr.copy(),
        "C_update": C_update.copy(),
