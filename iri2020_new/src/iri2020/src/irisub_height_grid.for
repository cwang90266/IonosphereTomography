...
C 2020.19 04/06/26 User select grid for height is introduced by Chunming Wang
C
C*****************************************************************
...
C                          HOURS
C         HEIGHT_Input  Array of selected heights.
C         NHeight       Integer number of selected heights, must be less than
C                       100.
C----- Removed        HEIBEG,       HEIGHT RANGE IN KM; maximal 100 heights, i.e.
C----- Removed         HEIEND,HEISTP        int((heiend-heibeg)/heistp)+1.le.100
C
...
C*****************************************************************
      ...
      REAL DHOUR
      INTEGER nummax
      PARAMETER (nummax=1000)
      INTEGER NHeight
      REAL HEIGHT_Input(nummax)
      REAL OUTF(20,nummax),OARR(100)      
...
!        write(*,*) 'Start of irisub'
        mess=jf(34)

!        write(*,*) 'Start initiation'
c set switches for NRLMSIS00cd
        ISW=0
        do KI=1,25
           SWMI(KI)=1.
        enddo
        
!        write(*,*) 'Middle initiation', nummax

        DO KI=1,20
           do kk=1,nummax
!              write(*,*) 'Middleinitiation', ki,kk
              OUTF(KI,kk)=-1.
           enddo
        enddo
...
!        write(*,*) 'Set the limits on height'
        heibeg=HEIGHT_Input(1)
        heiend=HEIGHT_Input(NHeight)
        numhei=NHeight
!        write(*,*) 'Set the limits on height done'
